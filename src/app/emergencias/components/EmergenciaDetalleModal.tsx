            ...
            <div className="bg-white/70 backdrop-blur-2xl backdrop-saturate-[1.2] w-full max-w-6xl rounded-[2.5rem] shadow-[0_8px_32px_0_rgba(0,0,0,0.12)] border border-white/60 h-[95vh] max-h-[900px] flex flex-col overflow-hidden">
                ...
                <div className="flex-1 min-h-0 flex overflow-hidden">
                    {isLoading ? (
                        ...
                        <div className="flex-1 flex items-center justify-center text-gray-500 font-medium text-sm">Emergencia no encontrada.</div>
                    ) : (() => {
                        ...
                        return (
                            <div className="flex-1 grid grid-cols-[3fr_2fr] divide-x divide-white/40 min-w-0">

                                {/* ── Left: Patient info + Status ── */}
                                <div className="p-5 flex flex-col gap-4 overflow-y-auto custom-scrollbar">

                                    {/* Patient header card */}
                                    <section className="bg-white/50 backdrop-blur-md border border-white/60 rounded-3xl p-5 shadow-sm shrink-0">
                                        <div className="flex justify-between items-start gap-3 mb-4">
                                            <div className="flex items-center gap-3 min-w-0">
                                                <div className="w-10 h-10 rounded-2xl bg-red-500/10 border border-red-400/20 flex items-center justify-center shrink-0">
                                                    <Siren size={18} className="text-red-500" />
                                                </div>
                                                <div className="min-w-0">
                                                    <h1 className="text-lg font-black text-gray-900 tracking-tight truncate">{pacienteName}</h1>
                                                    <div className="flex flex-wrap items-center gap-2.5 mt-0.5 text-xs text-gray-500 font-medium">
                                                        <span className="font-mono">{paDocumento || "Sin documento"}</span>
                                                        {paTelefono && <span className="flex items-center gap-1"><Phone size={11} /> {paTelefono}</span>}
                                                        <span className="font-mono text-gray-400">#Cita-{emergencia.citaId || emergencia.emergenciaId}</span>
                                                        <span className="flex items-center gap-1"><Clock size={11} /> {formatMilitaryTime(emergencia.fechaIngreso)}</span>
                                                    </div>
                                                </div>
                                            </div>
                                            <span className={`shrink-0 rounded-full px-3 py-1 text-xs font-black shadow-sm ${urgencyBadge[emergencia.nivelUrgencia] ?? "bg-gray-200 text-gray-700"}`}>
                                                {emergencia.nivelUrgencia}
                                            </span>
                                        </div>

                                        {/* Motivo */}
                                        <div className="bg-white/60 border border-white/70 rounded-2xl px-4 py-3 mb-3">
                                            <p className="text-[10px] font-bold text-gray-400/80 uppercase tracking-wider mb-1">Motivo de ingreso</p>
                                        ...
                                        <div className="bg-white/60 border border-white/70 rounded-2xl px-4 py-3">
                                            <div className="flex items-center justify-between gap-3">
                                                <div className="min-w-0">
                                                    <p className="text-[10px] font-bold text-gray-400/80 uppercase tracking-wider mb-1 flex items-center gap-1">
                                                        <Stethoscope size={10} /> Médico Asignado
                                                    </p>
                                                    {emergencia.medico && typeof emergencia.medico === 'object' ? (
                                                        <p className="text-sm font-bold text-gray-800">
                                                            ...
                                                            <span className="ml-1.5 text-xs font-medium text-gray-500">· {emergencia.medico.especialidad?.nombre || emergencia.medico.especialidad || "—"}</span>
                                                        </p>
                                                    ) : (
                                                ...
                                                <select
                                                    ...
                                                    className="shrink-0 px-3 py-2 rounded-xl bg-white/80 border border-white/80 text-xs font-bold text-gray-700 focus:outline-none focus:ring-2 focus:ring-red-400/50 transition-all shadow-sm max-w-[220px]"
                                                >
                                    ...
                                    {/* Status Timeline */}
                                    <section className="bg-white/50 backdrop-blur-md border border-white/60 rounded-3xl p-5 shadow-sm shrink-0">
                                        <h2 className="text-[10px] font-black text-gray-400/80 uppercase tracking-[0.2em] mb-3 flex items-center gap-2">
                                        ...
                                        <div className="flex items-center gap-2 flex-wrap">
                                            ...
                                            >
                                                <Ban size={13} /> Referido
                                            </button>
                                        </div>
                                    </section>
                                </div>

                                {/* ── Right: Payment + Insurance + Cartas ── */}
                                <div className="p-5 flex flex-col gap-4 overflow-y-auto custom-scrollbar">

                                    {/* Payment */}
                                    <section className="bg-white/50 backdrop-blur-md border border-white/60 rounded-3xl p-5 shadow-sm shrink-0">
                                        <h3 className="text-[10px] font-black text-gray-400/80 uppercase tracking-[0.2em] mb-3 flex items-center gap-2">
                                    ...
                                    <section className="bg-white/50 backdrop-blur-md border border-white/60 rounded-3xl p-5 shadow-sm shrink-0">
                                        <div className="flex items-center justify-between mb-3">
                                            <h3 className="text-[10px] font-black text-gray-400/80 uppercase tracking-[0.2em] flex items-center gap-2">
                                            ...
                                            {polizas.length > 0 && (
                                                <button
                                                    onClick={() => {
                                                        ...
                                                        if (currentAval) {
                                                            setCartaForm({ cartaAvalId: currentAval.cartaAvalId || "", polizaId: currentAval.polizaId, codigoAval: currentAval.codigoAval || "", estado: currentAval.estado || "SOLICITADA", observaciones: currentAval.observaciones || "" });
                                                        } else {
                                                            setCartaForm({ cartaAvalId: "", polizaId: polizas[0].polizaId, codigoAval: "", estado: "SOLICITADA", observaciones: "" });
                                                        }
                                        ...
                                        {polizas.length === 0 ? (
                                            <div className="bg-white/60 border-2 border-dashed border-white/80 rounded-2xl p-4 text-center">
                                            ...
                                            <div className="space-y-2">
                                                {polizas.map((p: any) => (
                                                    <div key={p.polizaId} className="bg-blue-50/50 border border-blue-200/50 rounded-2xl p-3.5">
                                                        <div className="flex items-center gap-2 mb-1">
                                                            <Building2 size={13} className="text-blue-600" />
                                                            <p className="font-bold text-blue-900 text-sm">{p.aseguradora.nombre}</p>
                                                        </div>
                                                        <p className="text-xs text-blue-700 font-mono"><Hash size={10} className="inline mr-0.5" />{p.numeroPoliza}</p>
                                                        {p.tipoCobertura && <p className="text-xs text-blue-600/80 mt-0.5"><Tag size={10} className="inline mr-1" />{p.tipoCobertura}</p>}
                                                    </div>
                                                ))}
                                            </div>
                                        )}
                                    ...
                                    {cartasAval.length > 0 && (
                                        <section className="bg-white/50 backdrop-blur-md border border-white/60 rounded-3xl p-5 shadow-sm shrink-0">
                                            <h3 className="text-[10px] font-black text-gray-400/80 uppercase tracking-[0.2em] mb-3 flex items-center gap-2">
                                                <FileText size={12} className="text-gray-400" /> Cartas Aval / Claves de Atención
                                            </h3>
                                            <div className="space-y-2">
                                                {cartasAval.map((c: any, index: number) => {
                                                    ...
                                                    return (
                                                        <div key={c.cartaAvalId || index} className={`border rounded-2xl p-3.5 flex items-center justify-between gap-3 ${cartaEstadoBadge[c.estado] ?? cartaEstadoBadge.SOLICITADA}`}>
                                                            <div className="min-w-0">
                                                                <p className="font-bold text-sm truncate">{asegInfo} {c.codigoAval ? `— ${c.codigoAval}` : ""}</p>
                                                                {c.observaciones && <p className="text-xs text-gray-500 mt-0.5 truncate">{c.observaciones}</p>}
                                                            </div>
