...
import { LayoutDashboard, ShoppingCart, LogOut, PackageSearch, History, ClipboardList, Hospital } from "lucide-react";
...
export default async function FarmaciaLayout({
    ...
    // Allowing both "ADMIN" and "FARMACIA" (mapped from INVENTORY or literal role name)
    const roleName = user?.role.toString().toUpperCase();
    if (!user || (roleName !== "ADMIN" && roleName !== "FARMACIA" && roleName !== "INVENTORY")) {
        redirect("/dashboard");
    }

    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-100 via-sky-50/30 to-blue-50/30 flex transition-colors duration-300">
            {/* Sidebar */}
            <aside className="fixed inset-y-0 left-0 z-50 w-64 bg-white/60 backdrop-blur-xl border-r border-white/50 shadow-[2px_0_16px_0_rgba(0,0,0,0.06)] hidden lg:flex overflow-hidden flex flex-col">
                ...
                <div className="flex-1 p-3 space-y-0 overflow-y-auto custom-scrollbar scroll-smooth">
                    ...
                    <NavItem href="/farmacia" icon={<ClipboardList size={16} />} label="Bandeja de Solicitudes" />
                    ...
                    <NavItem href="/farmacia/insumos" icon={<PackageSearch size={16} />} label="Gestión Insumos" />
                    <NavItem href="/farmacia/historial" icon={<History size={16} />} label="Historial Despachos" />
                    ...
                    <NavItem href="/almacen" icon={<Hospital size={16} />} label="Ir a Almacén" />
                ...
                <div className="shrink-0 p-4 border-t border-white/40">
                    <AdminProfile 
                        user={{ name: user.name }} 
                        role={roleName === 'ADMIN' ? 'Administrador' : 'Encargado Farmacia'} 
                    />
                </div>
            </aside>
            ...
            <div className="flex-1 lg:ml-64 flex flex-col min-h-screen">
                ...
                <header className="flex lg:hidden items-center justify-between sticky top-0 z-40 bg-white/30 backdrop-blur-md border-b border-white/40 p-4">
                    <span className="text-lg font-black text-gray-900 tracking-tight underline decoration-[#a1db4b] decoration-4 underline-offset-4">Panel Farmacia</span>
                    <ExchangeRateWidget />
                </header>
