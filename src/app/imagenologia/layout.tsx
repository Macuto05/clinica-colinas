...
export default function ImagenologiaLayout({ children }: ImagenologiaLayoutProps) {
    ...
    useEffect(() => {
        if (!loading) {
            ...
            if (roleName !== "ADMIN" && roleName !== "IMAGENOLOGIA") {
    ...
    const navigation = [
        { name: "Bandeja de Imágenes", href: "/imagenologia", icon: Activity },
    ];
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-100 via-lime-50/30 to-green-50/30">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-sm font-bold text-gray-500 tracking-wide">Cargando módulo de imagenología…</p>
                </div>
            </div>
        );
    }
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-gray-900 truncate">
                                {(user as any)?.name || user?.email || "Imagenología"}
                            </p>
                            <p className="text-xs text-gray-500 truncate">
                                {user?.role === 'ADMIN' ? 'Administrador' : 'Técnico Imagenólogo'}
                            </p>
                        </div>
                    </div>
                </div>
