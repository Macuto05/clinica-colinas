...
import { ClipboardList, User, CheckCircle2, AlertCircle, ArrowRight, X, Loader2, RefreshCcw, FlaskConical, Filter, UploadCloud } from "lucide-react";
...
export default function InboxSolicitudes() {
    ...
    // Processing State for specific exam
    ...
    const fetchSolicitudes = async (mode: "nuevas" | "historial") => {
        setIsLoading(true);
        try {
            let url = "/api/laboratorio/solicitudes?group=patient";
            if (mode === "historial") {
                url += "&history=true";
            }
    ...
    const handleAtenderExamen = async (solicitudId: string, detalleLabId: string) => {
        setIsUploading(true);
        try {
            ...
            setActionStatusMap(prev => ({ ...prev, [detalleLabId]: { type: 'success', message: "Resultado Guardado" } }));
            ...
            setActionStatusMap(prev => ({ ...prev, [detalleLabId]: { type: 'error', message: error.message } }));
        } finally {
            ...
            <div className="lg:col-span-4 flex flex-col gap-4 overflow-y-auto max-h-[80vh] custom-scrollbar pr-2">
                ...
                <div className="flex items-center justify-between mb-2">
                    <h2 className="text-xs font-black text-gray-400 uppercase tracking-widest flex items-center gap-2">
                        ...
                        {viewMode === "nuevas" ? "Pacientes con Pedidos" : "Historial"}
                    </h2>
                ...
                <AnimatePresence mode="popLayout">
                    {groups.length === 0 ? (
                        ...
                        >
                            ...
                            <p className="text-gray-400 font-bold text-sm">No hay peticiones actualmente.</p>
                        </motion.div>
                    ) : (
                        groups.map((g) => (
                            <motion.button
                    ...
                    {selectedGroup ? (
                            ...
                            <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
                                <div className="space-y-6">
                                    {selectedGroup.solicitudes.map((sol) => {
                                        const isExpanded = expandedRequests.has(sol.solicitudLabId);
                                        const unAttendedCount = sol.examenes.filter(e => !e.atendido).length;

                                        return (
                                            <div key={sol.solicitudLabId} 
                                                ...
                                                <div className="relative">
                                                    <button 
                                                        onClick={() => toggleRequest(sol.solicitudLabId)}
                                                    ...
                                                    >
                                                        <div className="flex items-center gap-4">
                                                            ...
                                                            <div>
                                                                <div className="flex items-center gap-3">
                                                                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Petición #{sol.solicitudLabId}</p>
                                                                    {viewMode === 'nuevas' && (
                                                        ...
                                                        <div className="text-right flex items-center gap-4">
                                                            <div className="hidden sm:block">
                                                                <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest leading-none">Exámenes</p>
                                                        ...
                                                        >
                                                            <div className="px-5 pb-5 pt-2 space-y-4 border-t border-white/40">
                                                                {sol.examenes.map((examen) => (
                                                                    <div key={examen.detalleLabId} className="flex flex-col p-4 bg-white/50 border border-white/60 rounded-2xl">
                                                                        <div className="flex items-center justify-between mb-2">
                                                                            <div className="flex items-center gap-3">
                                                                            ...
                                                                            {viewMode === "nuevas" && !examen.atendido && processingDetalleId !== examen.detalleLabId && (
                                                                                <button
                                                                                    onClick={() => {
                                                                                        setProcessingDetalleId(examen.detalleLabId);
                                                                        ...
                                                                        {/* Inline Upload Form */}
                                                                        <AnimatePresence>
                                                                            {processingDetalleId === examen.detalleLabId && !examen.atendido && (
                                                                                <motion.div 
                                                                                    initial={{ height: 0, opacity: 0 }}
                                                                                    animate={{ height: "auto", opacity: 1 }}
                                                                                    exit={{ height: 0, opacity: 0 }}
                                                                                ...
                                                                                >
                                                                                    <div className="space-y-4">
                                                                                        <div>
                                                                                            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1 shadow-none block">Observaciones / Conclusión del Técnico (Opcional)</label>
                                                                                            <textarea 
                                                                                                ...
                                                                                                placeholder="Escribe alguna acotación..."
                                                                                            />
                                                                                        </div>
                                                                                        ...
                                                                                        <div className="flex items-center gap-3">
                                                                                            <label className="flex-1 border-2 border-dashed border-gray-200 rounded-xl p-4 flex flex-col items-center justify-center text-center cursor-pointer hover:border-lime-400 hover:bg-lime-50/50 transition-all">
                                                                                                ...
                                                                                                <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest">
                                                                                                    {uploadForm.fileName || "Adjuntar PDF o Imagen"}
                                                                                                </span>
                                                                                            ...
                                                                                            <div className="flex flex-col gap-2">
                                                                                                <button
                                                                                                    ...
                                                                                                    onClick={() => handleAtenderExamen(sol.solicitudLabId, examen.detalleLabId)}
                                                                                        ...
                                                                                        {actionStatusMap[examen.detalleLabId] && (
                                                                                            <p className={`text-[10px] font-black uppercase tracking-widest text-center mt-2 ${actionStatusMap[examen.detalleLabId]?.type === 'error' ? 'text-red-500' : 'text-emerald-500'}`}>
                                                                                                {actionStatusMap[examen.detalleLabId]?.message}
                                                                                            </p>
                                                                                        )}
                                                                                    </div>
                                                                                </motion.div>
                        ...
                        <div className="h-full flex flex-col items-center justify-center text-center p-12 bg-white/30 backdrop-blur-sm border border-dashed border-white/60 rounded-[2.5rem]">
                            ...
                            <p className="text-gray-400 text-[10px] uppercase font-black mt-2 tracking-widest max-w-[200px]">
                                Selecciona una tarjeta para ver las peticiones médicas
                            </p>
                        </div>
                    )}
                </AnimatePresence>
