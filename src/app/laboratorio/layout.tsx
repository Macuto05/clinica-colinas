...
export default function LaboratorioLayout({ children }: LaboratorioLayoutProps) {
    ...
    useEffect(() => {
        if (!loading) {
            ...
            if (roleName !== "ADMIN" && roleName !== "LABORATORIO") {
    ...
    const navigation = [
        { name: "Bandeja de Exámenes", href: "/laboratorio", icon: FlaskConical },
    ];
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-100 via-lime-50/30 to-green-50/30">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-sm font-bold text-gray-500 tracking-wide">Cargando módulo de laboratorio…</p>
                </div>
            </div>
        );
    }
                ...
                <div className="p-4 border-b border-white/50 bg-white/30 backdrop-blur-md shrink-0">
                    <div className="flex items-center gap-3">
                        <div className="h-10 w-10 rounded-full bg-lime-500/10 flex items-center justify-center text-lime-700 font-black shadow-inner border border-lime-500/20 backdrop-blur-md">
                            {user?.name?.charAt(0) || user?.email?.charAt(0)?.toUpperCase() || "L"}
                        </div>
                        <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-gray-900 truncate">
                                {(user as any)?.name || user?.email || "Laboratorio"}
                            </p>
