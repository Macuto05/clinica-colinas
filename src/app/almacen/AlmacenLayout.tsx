        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-100 via-lime-50/30 to-green-50/30">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-sm font-bold text-gray-500 tracking-wide">Cargando módulo de almacén…</p>
                </div>
            </div>
        );
    }
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-100 via-green-50/30 to-lime-50/30 flex">
            ...
            >
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-gray-900 truncate">
                                {(user as any)?.name || user?.email || "Almacén"}
                            </p>
                            <p className="text-xs text-gray-500 truncate">
                                {user?.role === 'ADMIN' ? 'Administrador' : 'Almacenista'}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto custom-scrollbar">
                    <div className="mb-2">
                        <p className="px-3 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-1.5 mt-2">Almacén</p>
                        {navigation.filter(n => n.section === "Almacén").map(item => {
                            const active = item.href === "/almacen" ? pathname === "/almacen" : isActive(item.href);
                            return (
            ...
            <div className="flex-1 flex flex-col min-w-0 overflow-hidden lg:pl-56">
                ...
                <header className="lg:hidden h-16 bg-white/60 backdrop-blur-xl border-b border-white/50 flex items-center justify-between px-4">
                    ...
                    <span className="font-semibold text-gray-900 truncate max-w-[150px]">
                        {navigation.find(n => isActive(n.href))?.name || "Panel Almacén"}
                    </span>
