...
export default function CajaLayout({ children }: CajaLayoutProps) {
    ...
    const navigation = [
        { name: "Resumen", href: "/caja", icon: LayoutDashboard, section: "Caja Principal" },
        { name: "Aprobación Compras", href: "/caja/compras", icon: ShoppingCart, section: "Gestión" },
        { name: "Proveedores", href: "/caja/proveedores", icon: Users, section: "Gestión" },
        { name: "Gestión de Pagos", href: "/caja/pagos", icon: ShoppingCart, section: "Gestión" },
        { name: "Configuración", href: "/caja/configuracion", icon: Settings, section: "Gestión" },
    ];
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-100 via-lime-50/30 to-green-50/30">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-sm font-bold text-gray-500 tracking-wide">Cargando módulo de caja…</p>
                </div>
            </div>
        );
    }
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-100 via-green-50/30 to-lime-50/30 flex">
            ...
            >
                ...
                <div className="p-4 border-b border-white/50 bg-white/30 backdrop-blur-md shrink-0">
                    <div className="flex items-center gap-3">
                        <div className="h-10 w-10 rounded-full bg-lime-500/10 flex items-center justify-center text-lime-700 font-black shadow-inner border border-lime-500/20 backdrop-blur-md">
                            {user?.name?.charAt(0) || user?.email?.charAt(0)?.toUpperCase() || "C"}
                        </div>
                        <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-gray-900 truncate">
                                {(user as any)?.name || user?.email || "Caja"}
                            </p>
                ...
                <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
                    <div className="mb-2">
                        <p className="px-3 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-1.5 mt-2">Caja Principal</p>
                        {navigation.filter(n => n.section === "Caja Principal").map(item => (
            ...
            <div className="flex-1 flex flex-col min-w-0 overflow-hidden lg:pl-56">
                ...
                <header className="lg:hidden h-16 bg-white/60 backdrop-blur-xl border-b border-white/50 flex items-center justify-between px-4">
                    ...
                    <span className="font-semibold text-gray-900 truncate max-w-[150px]">
                        {navigation.find(n => isActive(n.href))?.name || "Panel Caja"}
                    </span>
